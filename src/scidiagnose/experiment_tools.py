...
COSTS={"inspect":1,"compare":2,"transform_and_compare":4,"shift_and_compare":4}
class ExperimentTools:
    ...
    def _ensure_data(self) -> None:
        ...
        base=f"{self.executor.workspace}/inputs/geo_001"; self.executor._ssh(f"mkdir -p {base}")
    ...
    def execute(self, tool: str, arguments: dict[str,Any] | None=None) -> dict[str,Any]:
        ...
        self._ensure_data(); assert isinstance(self.executor,SSHDirectExecutor); job_dir=self.executor.create_job_dir(exp_id); self.executor.upload(local,f"{job_dir}/experiment.json")
        job=self.executor.submit(exp_id,[self.executor.remote_python,f"{self.executor.workspace}/scripts/run_experiment.py","--input-dir",f"{self.executor.workspace}/inputs/geo_001","--experiment-json","experiment.json","--output-json","result.json"])
        ...
        record={"experiment_id":exp_id,"tool":tool,"arguments":arguments or {},"backend":job.backend,"remote_host":job.remote_host,"remote_pid":job.remote_pid,"job_id":job.job_id,"status":status,"cost":COSTS[tool],"result":result,"stdout":stdout,"stderr":stderr}; (self.run_dir/"experiments"/f"{exp_id}.json").write_text(json.dumps(record,indent=2)); return record
