...
def write_case(name:str,recipe:list[dict[str,object]],repair:list[dict[str,object]],seed:int)->dict[str,float]:
    ...
    for n,a in {"reference.npy":ref,"target_clean.npy":clean,"target_faulty.npy":target}.items():np.save(data/n,a)
...
def main()->None:
    ...
    specs=[("b01",[{"type":"transform","operation":"rot180"}],[{"type":"transform","operation":"rot180"}]),("b02",[{"type":"shift","dr":1,"dc":-2},{"type":"transform","operation":"rot180"}],[{"type":"transform","operation":"rot180"},{"type":"shift","dr":1,"dc":-2}]),("b03",[],[])]
