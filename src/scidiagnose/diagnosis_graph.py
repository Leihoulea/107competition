...
class DiagnosisGraph:
    def __init__(self,agent:ManualAgent|OpenAICompatibleAgent,tools:ExperimentTools,run_dir:Path)->None:
        ...
        g.add_edge(START,'observe');g.add_edge('observe','hypothesize');g.add_edge('hypothesize','plan');g.add_edge('plan','execute');g.add_edge('execute','evidence');g.add_edge('evidence','reflect');g.add_conditional_edges('reflect',lambda s:'finalize' if s['diagnosis_status'] in {'validated_fault','no_fault','budget_exhausted'} else 'hypothesize',{'finalize':'finalize','hypothesize':'hypothesize'});g.add_edge('finalize',END);self.app=g.compile(checkpointer=self.checkpointer)
    def log(self,node:str,**data:Any)->None:self.trace.open('a').write(json.dumps({'node':node,'timestamp':time.time(),**data})+'\n')
    def observe(self,s:DiagnosisGraphState)->dict[str,Any]:self.log('observe',initial=s['initial_observation']);return {'diagnosis_status':'investigating'}
    ...
    def plan(self,s:DiagnosisGraphState)->dict[str,Any]:
        ...
        if action.type=='final':return {'current_plan':{'final':action.final},'diagnosis_status':'budget_exhausted'}
    ...
    def finalize(self,s:DiagnosisGraphState)->dict[str,Any]:
        best=max(s.get('experiments',[]),key=lambda x:x['result'].get('metrics',{}).get('agreement',0),default=None);final={'decision':'fault' if s['diagnosis_status']=='validated_fault' else 'no_fault','fault_family':'validated_candidate' if best else 'undetermined','root_cause':'evidence-backed candidate' if best else 'budget exhausted','confidence':.8,'evidence_experiment_ids':[best['experiment_id']] if best else [],'recommended_repair':best['arguments'] if best else {}};self.log('finalize',final=final);return {'final_diagnosis':final}
